...
def display_infos( bins, coinc, nocoinc, single, dir, filename):

    maxi = bins[list(single).index(max(single))]

    E_coinc, RMS_coinc, N_coinc = infos(coinc, bins, maxi)
    E_nocoinc, RMS_nocoinc, N_nocoinc = infos(nocoinc, bins, maxi)
    E_single, RMS_single, N_single = infos(single, bins, maxi)
...
def process_file(dir, nuclei, b):
    ...
    for dir in ["Up", "Down"]:
        ...
        res.append(display_infos(x, sum_dir_coinc, sum_dir_nocoinc, sum_dir_single, dir, filename))
    return res[0], res[1]
...
for nuclei in list_nuclei:
    ...
    for b in list_b:
        result.append(pool.apply_async(process_file, args=(dir, nuclei, b)))
    pool.close()
    pool.join()
    ...
    for j, b in enumerate(list_b):
        ...
        for i, dir in enumerate(["Up", "Down"]):
            dic_shift[nuclei][b][dir] = result[j][i]
...
for dir in ["Up", "Down"]:
    ...
    res = (Mg-Ar)/Ar*100
    err = np.sqrt((err_Mg/Ar)**2+(err_Ar*Mg/Ar**2)**2)*100
