...
G4VPhysicalVolume *Wisard_Detector::Construct()
{
  ...
  // G4double outerRadius = 6.5 * cm;
   ...
   G4double SuppCatcher_Plate_height = H;
    ...
    thicknessMylarSource_central = 0.5 * um;
  ...
  dic_strip[1] = std::make_tuple(SiDet_Strip_1, G4ThreeVector(0, -pDy1 / 2 + spazio_tra_Bordo_e_strip5 + y_SiDet_Strip_5 + spazio_tra_Strip + y_SiDet_Strip_4 + spazio_tra_Strip + y_SiDet_Strip_3 + spazio_tra_Strip + y_SiDet_Strip_2 + spazio_tra_Strip + y_SiDet_Strip_1 / 2, -thicknessSiDetectorGrid/2), SiDet_Strip_1_dl, SiDet_Strip_1_grid);
  dic_strip[2] = std::make_tuple(SiDet_Strip_2, G4ThreeVector(0, -pDy1 / 2 + spazio_tra_Bordo_e_strip5 + y_SiDet_Strip_5 + spazio_tra_Strip + y_SiDet_Strip_4 + spazio_tra_Strip + y_SiDet_Strip_3 + spazio_tra_Strip + y_SiDet_Strip_2 / 2, -thicknessSiDetectorGrid/2), SiDet_Strip_2_dl, SiDet_Strip_2_grid);
  dic_strip[3] = std::make_tuple(SiDet_Strip_3, G4ThreeVector(0, -pDy1 / 2 + spazio_tra_Bordo_e_strip5 + y_SiDet_Strip_5 + spazio_tra_Strip + y_SiDet_Strip_4 + spazio_tra_Strip + y_SiDet_Strip_3 / 2, -thicknessSiDetectorGrid/2), SiDet_Strip_3_dl, SiDet_Strip_3_grid);
...
void Wisard_Detector::SetSiDeadLayer_Thickness(G4double value)
{
  ...
  for (int i = 1; i <= 8; i++)
  {
      ...
      get<8>(tab[i-1]).second->SetTranslation(G4ThreeVector(0., 0., thicknessSiDetector / 2 - value / 2));
      get<9>(tab[i-1]).second->SetTranslation(G4ThreeVector(0., 0., thicknessSiDetector / 2 - value / 2));
      get<10>(tab[i-1]).second->SetTranslation(G4ThreeVector(0., 0., thicknessSiDetector / 2 - value / 2));
      get<11>(tab[i-1]).second->SetTranslation(G4ThreeVector(0., 0., thicknessSiDetector / 2 - value / 2));
  }
...
void Wisard_Detector::SetCatcher_Thickness(G4double Al1_e, G4double Mylar_e, G4double Al2_e)
{
  ...
  Physics_AlSource1_central->SetCopyNo(1);
  Physics_MylarSource_central->SetCopyNo(2);
  Physics_AlSource2_central->SetCopyNo(3);
...
void Wisard_Detector::SetCatcherPosition_theta(G4String position, G4double value)
{
  ...
  {
    G4Exception("Wisard_Detector", ("Wrong Catcher position : " + position).c_str(), JustWarning, "");
  }
