...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  File:   MyG4RadioactiveDecay.cc                                             //
//  Author: D.H. Wright (SLAC)                                                //
//  Date:   9 August 2017                                                     //
//  Description: version the MyG4RadioactiveDecay process by F. Lei and         //
...
const G4double MyG4RadioactiveDecay::levelTolerance = 10.0*eV;
const G4ThreeVector MyG4RadioactiveDecay::origin(0.,0.,0.);

#ifdef G4MULTITHREADED
#include "G4AutoLock.hh"
G4Mutex MyG4RadioactiveDecay::radioactiveDecayMutex = G4MUTEX_INITIALIZER;
DecayTableMap* MyG4RadioactiveDecay::master_dkmap = 0;

G4int& MyG4RadioactiveDecay::NumberOfInstances()
{
...
MyG4RadioactiveDecay::MyG4RadioactiveDecay(const G4String& processName)
...
{
  // G4cout << "\033[1;32m" << "My custom radioactive decay process is being used" << "\033[0m" << G4endl; 
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 1) {
    G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay constructor: processName = " << processName
           << "\033[0m" << G4endl; 
  }
#endif
  ...
  theRadioactiveDecayMessenger = new MyG4RadioactiveDecayMessenger(this);
  ...
  if (!path_var) {
    G4Exception("MyG4RadioactiveDecay()","HAD_RDM_200",FatalException,
    ...
    if (!testFile.is_open() )
      G4Exception("MyG4RadioactiveDecay()","HAD_RDM_201",FatalException,
...
#ifdef G4MULTITHREADED
  G4AutoLock lk(&MyG4RadioactiveDecay::radioactiveDecayMutex);
  NumberOfInstances()++;
...
void MyG4RadioactiveDecay::ProcessDescription(std::ostream& outFile) const
{
  outFile << "The radioactive decay process (MyG4RadioactiveDecay) handles the\n"
...
MyG4RadioactiveDecay::~MyG4RadioactiveDecay()
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::~MyG4RadioactiveDecay" << "\033[0m" << G4endl; 
...
G4bool MyG4RadioactiveDecay::IsApplicable(const G4ParticleDefinition& aParticle)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::IsApplicable" << "\033[0m" << G4endl; 
...
G4DecayTable* MyG4RadioactiveDecay::GetDecayTable(const G4ParticleDefinition* aNucleus)
{
...
void MyG4RadioactiveDecay::SelectAVolume(const G4String& aVolume)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::SelectAVolume" << "\033[0m" << G4endl; 
  ...
  if (volume != nullptr)
  {
    ...
    if (GetVerboseLevel() > 0)
      G4cout << "\033[1;32m" <<" Radioactive decay applied to " << aVolume << "\033[0m" << G4endl; 
  }
  else
  {
    ...
    ed << aVolume << " is not a valid logical volume name."
       ...
       << "\033[0m" << G4endl; 
    G4Exception("MyG4RadioactiveDecay::SelectAVolume()", "HAD_RDM_300",
                JustWarning, ed);
  }
}


void MyG4RadioactiveDecay::DeselectAVolume(const G4String& aVolume)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::DeselectAVolume" << "\033[0m" << G4endl; 
  G4LogicalVolumeStore* theLogicalVolumes = G4LogicalVolumeStore::GetInstance();
  G4LogicalVolume* volume = nullptr;
  volume = theLogicalVolumes->GetVolume(aVolume);
  if (volume != nullptr)
  {
    ...
    if (location != ValidVolumes.cend() )
    {
      ...
      if (GetVerboseLevel() > 0)
        G4cout << "\033[1;32m" <<" MyG4RadioactiveDecay::DeselectAVolume: " << aVolume
               << " is removed from list " << "\033[0m" << G4endl; 
    }
    else
    {
      G4ExceptionDescription ed;
      ed << aVolume << " is not in the list.  No action taken." << "\033[0m" << G4endl; 
      G4Exception("MyG4RadioactiveDecay::DeselectAVolume()", "HAD_RDM_300",
                  JustWarning, ed);
    }
  }
  else
...
void MyG4RadioactiveDecay::SelectAllVolumes() 
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::SelectAllVolumes" << "\033[0m" << G4endl; 
  G4LogicalVolumeStore* theLogicalVolumes = G4LogicalVolumeStore::GetInstance();
  G4LogicalVolume* volume = nullptr;
  ValidVolumes.clear();
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1)
    G4cout << "\033[1;32m" <<" RDM Applies to all Volumes"  << "\033[0m" << G4endl; 
#endif
...
#ifdef G4VERBOSE
    if (GetVerboseLevel()>1)
      G4cout << "\033[1;32m" <<"       RDM Applies to Volume " << volume->GetName() << "\033[0m" << G4endl; 
#endif
  }
  std::sort(ValidVolumes.begin(), ValidVolumes.end());
...
void MyG4RadioactiveDecay::DeselectAllVolumes() 
{
  ValidVolumes.clear();
  isAllVolumesMode=false;
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 1) G4cout << "\033[1;32m" <<"RDM removed from all volumes" << "\033[0m" << G4endl;  
#endif
}
...
G4double MyG4RadioactiveDecay::GetMeanLifeTime(const G4Track& theTrack,
                                                 G4ForceCondition*)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::GetMeanLifeTime" << "\033[0m" << G4endl; 
  G4double meanlife = 0.;
  ...
  if (GetVerboseLevel() > 2) {
    G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::GetMeanLifeTime() " << "\033[0m" << G4endl; 
    G4cout << "\033[1;32m" <<"KineticEnergy: " << theParticle->GetKineticEnergy()/GeV
           << " GeV, Mass: " << theParticle->GetMass()/GeV
           << " GeV, Life time: " << theLife/ns << " ns " << "\033[0m" << G4endl; 
  }
#endif
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2)
    G4cout << "\033[1;32m" <<" mean life time: " << meanlife/s << " s " << "\033[0m" << G4endl; 
#endif
...
G4double MyG4RadioactiveDecay::GetMeanFreePath(const G4Track& aTrack, G4double,
                                             G4ForceCondition*)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::GetMeanFreePath" << "\033[0m" << G4endl; 
  ...
  if (GetVerboseLevel() > 2) {
    G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::GetMeanFreePath() " << "\033[0m" << G4endl; 
    G4cout << "\033[1;32m" <<"  KineticEnergy: " << aParticle->GetKineticEnergy()/GeV
  ...
  if (tau != -1) {
    ...
    if (tau < -1000.0) {
      ...
      G4cout << "\033[1;32m" <<aParticleDef->GetParticleName() << " has lifetime " << tau << "\033[0m" << G4endl; 
      G4ExceptionDescription ed;
      ed << "Ion has negative lifetime " << tau
         << " but is not stable.  Setting mean free path to DBL_MAX" << "\033[0m" << G4endl;  
      G4Exception("MyG4RadioactiveDecay::GetMeanFreePath()", "HAD_RDM_011",
                   JustWarning, ed);
      pathlength = DBL_MAX;
...
#ifdef G4VERBOSE
        if (GetVerboseLevel() > 2) {
          G4cout << "\033[1;32m" <<"G4Decay::GetMeanFreePath: "
                 << aParticleDef->GetParticleName()
                 << " stops, kinetic energy = "
                 << aParticle->GetKineticEnergy()/keV <<" keV " << "\033[0m" << G4endl; 
        }
#endif
      // }
    }
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2) {
    G4cout << "\033[1;32m" <<"mean free path: "<< pathlength/m << " m" << "\033[0m" << G4endl; 
  }
#endif
  return  pathlength;
}
...
void MyG4RadioactiveDecay::BuildPhysicsTable(const G4ParticleDefinition&)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::BuildPhysicsTable" << "\033[0m" << G4endl; 
  if (!isInitialised) {
    isInitialised = true;
#ifdef G4VERBOSE
...
void
MyG4RadioactiveDecay::StreamInfo(std::ostream& os, const G4String& endline)
{
  G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::StreamInfo" << "\033[0m" << G4endl; 
...
G4DecayTable*
MyG4RadioactiveDecay::LoadDecayTable(const G4ParticleDefinition& theParentNucleus) 
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::LoadDecayTable" << "\033[0m" << G4endl; 
  ...
  G4cout << "\033[1;32m" <<"Loading decay table for " << theParentNucleus.GetParticleName()
         << ", A = " << A << ", Z = " << Z
         << ", excitation energy = " << levelEnergy << " keV" << "\033[0m" << G4endl;
  ...
  if (DecaySchemeFile.good()) {
    ...
    while (!complete && !DecaySchemeFile.getline(inputChars, 120).eof()) {  /* Loop checking, 01.09.2015, D.Wright */
      loop++;
      if (loop > 100000) {
        G4Exception("MyG4RadioactiveDecay::LoadDecayTable()", "HAD_RDM_100",
      ...
      if (inputChars[0] != '#' && inputLine.length() != 0) {
        ...
        if (inputChars[0] == 'P') {
          ...
          tmpStream >> recordType >> parentExcitation >> floatingFlag >> halflife;
          G4cout << "\033[1;32m" <<"Parent record: " << recordType << ", excitation energy: "
                 << parentExcitation*keV << " keV, floating flag: " << floatingFlag
                 << ", halflife: " << G4BestUnit(halflife, "Time") << "\033[0m" << G4endl;
          ...
          if (inputLine.length() < 72) {
            ...
            G4cout << "\033[1;32m" <<"Decay mode: " << theDecayMode
                   << ", total BR: " << decayModeTotal << "\033[0m" << G4endl;
            switch (theDecayMode) {
              case IT:
                {
...
//                anITChannel->DumpNuclearInfo();
              ...
              default:
                G4Exception("MyG4RadioactiveDecay::LoadDecayTable()", "HAD_RDM_000",
            ...
            if (inputLine.length() < 84) {
              ...
              G4cout << "\033[1;32m" <<"Decay mode: " << theDecayMode
                     << ", a: " << a << ", b: " << b << ", c: " << c
                     << ", daughter float flag: " << daughterFloatFlag
                     << "\033[0m" << G4endl;
            } else {
...
//                aBetaMinusChannel->SetHLThreshold(halflifethreshold);
              ...
              case BetaPlus:
              {
                MyG4BetaPlusDecay* aBetaPlusChannel =
                  new MyG4BetaPlusDecay(&theParentNucleus, b, c*MeV, a*MeV,
                                      daughterFloatLevel, betaType);//, ConvertingHalflifeToWidth(halflife));
	      ...
	      case Proton:
              {
                G4ProtonDecay* aProtonChannel =
                  new G4ProtonDecay(&theParentNucleus, b, c*MeV, a*MeV,
                                    daughterFloatLevel);//, ConvertingHalflifeToWidth(halflife));
...
//                  new G4SFDecay(&theParentNucleus, decayModeTotal, 0.0, 0.0);
              ...
              case BDProton:
                  // Not yet implemented
                  // G4cout << "\033[1;32m" <<" beta-delayed proton decay, a = " << a << ", b = " << b << ", c = " << c << "\033[0m" << G4endl; 
                  break;

              case BDNeutron:
                  // Not yet implemented
                  // G4cout << "\033[1;32m" <<" beta-delayed neutron decay, a = " << a << ", b = " << b << ", c = " << c << "\033[0m" << G4endl; 
                  break;

              case Beta2Minus:
                  // Not yet implemented
                  // G4cout << "\033[1;32m" <<" Double beta- decay, a = " << a << ", b = " << b << ", c = " << c << "\033[0m" << G4endl; 
                  break;
              ...
              case Proton2:
                  // Not yet implemented
                  // G4cout << "\033[1;32m" <<" Double proton decay, a = " << a << ", b = " << b << ", c = " << c << "\033[0m" << G4endl; 
                  break;
...
void
MyG4RadioactiveDecay::AddUserDecayDataFile(G4int Z, G4int A, G4String filename)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::AddUserDecayDataFile" << "\033[0m" << G4endl; 
  if (Z < 1 || A < 2) G4cout << "\033[1;32m" <<"Z and A not valid!" << "\033[0m" << G4endl; 
  ...
  if (DecaySchemeFile) {
    ...
    G4ExceptionDescription ed;
    ed << filename << " does not exist! " << "\033[0m" << G4endl; 
    G4Exception("MyG4RadioactiveDecay::AddUserDecayDataFile()", "HAD_RDM_001",
                FatalException, ed);
  }
}
...
G4VParticleChange*
MyG4RadioactiveDecay::DecayIt(const G4Track& theTrack, const G4Step&)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::DecayIt" << "\033[0m" << G4endl; 
...
#ifdef G4VERBOSE
      if (GetVerboseLevel()>1) {
        G4cout <<"MyG4RadioactiveDecay::DecayIt : "
               << theTrack.GetVolume()->GetLogicalVolume()->GetName()
               << " is not selected for the RDM"<< "\033[0m" << G4endl; 
        G4cout << "\033[1;32m" <<" There are " << ValidVolumes.size() << " volumes" << "\033[0m" << G4endl; 
        G4cout << "\033[1;32m" <<" The Valid volumes are " << "\033[0m" << G4endl; 
        for (std::size_t i = 0; i< ValidVolumes.size(); ++i)
                                  G4cout << "\033[1;32m" <<ValidVolumes[i] << "\033[0m" << G4endl; 
      }
#endif
...
#ifdef G4VERBOSE
    if (GetVerboseLevel() > 1) {
      G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::DecayIt : "
...
#ifdef G4VERBOSE
    if (GetVerboseLevel()>1) {
      G4cout <<"MyG4RadioactiveDecay::DecayIt : Decay vertex :";
...
void MyG4RadioactiveDecay::DecayAnalog(const G4Track& theTrack)
{
  if (theTrack.GetParentID() == 0) last_delta = 0.0;
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::DecayAnalog" << "\033[0m" << G4endl; 
  const G4DynamicParticle* theParticle = theTrack.GetDynamicParticle();
  const G4ParticleDefinition* theParticleDef = theParticle->GetDefinition();
  G4DecayProducts* products = DoDecay(*theParticleDef);
  ...
  if (GetVerboseLevel() > 1) {
    G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::DecayAnalog: Decay vertex :";
    G4cout << "\033[1;32m" <<" Time: " << finalGlobalTime/ns << "[ns]";
    G4cout << "\033[1;32m" <<" X:" << (theTrack.GetPosition()).x() /cm << "[cm]";
    G4cout << "\033[1;32m" <<" Y:" << (theTrack.GetPosition()).y() /cm << "[cm]";
    G4cout << "\033[1;32m" <<" Z:" << (theTrack.GetPosition()).z() /cm << "[cm]";
    G4cout << "\033[1;32m" <<G4endl;
    G4cout << "\033[1;32m" <<"G4Decay::DecayIt : decay products in Lab. Frame" << "\033[0m" << G4endl; 
    products->DumpInfo();
    products->IsChecked();
  }
...
G4DecayProducts*
MyG4RadioactiveDecay::DoDecay(const G4ParticleDefinition& theParticleDef)
{
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::DoDecay" << "\033[0m" << G4endl; 
  ...
  if (theDecayChannel == 0) {
    ...
    G4ExceptionDescription ed;
    ed << " Cannot determine decay channel for " << theParticleDef.GetParticleName() << "\033[0m" << G4endl; 
    G4Exception("MyG4RadioactiveDecay::DoDecay", "HAD_RDM_013",
                FatalException, ed);
  } else {
    // A decay channel has been identified, so execute the DecayIt.
#ifdef G4VERBOSE
    if (GetVerboseLevel() > 1) {
      G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::DoIt : selected decay channel addr: "
             << theDecayChannel << "\033[0m" << G4endl; 
    }
#endif
    theRadDecayMode = (static_cast<G4NuclearDecay *>(theDecayChannel))->GetDecayMode();
    // if (theRadDecayMode == 8)
    // {
    //   MyG4ProtonDecay *aProtonChannel = dynamic_cast<MyG4ProtonDecay *>(theDecayChannel);
    //   products = aProtonChannel->DecayIt(theParticleDef.GetPDGMass(), last_delta);

    //   last_delta = aProtonChannel->GetDelta();
    // }
    // else if (theRadDecayMode == 2)
    // {
    //   MyG4BetaPlusDecay *aBetaPlusChannel = dynamic_cast<MyG4BetaPlusDecay *>(theDecayChannel);
    //   products = aBetaPlusChannel->DecayIt(theParticleDef.GetPDGMass(), last_delta);

    //   last_delta = aBetaPlusChannel->GetDelta();
    // } 
    // else
    // {
...
void MyG4RadioactiveDecay::CollimateDecay(G4DecayProducts* products) {
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 1) G4cout << "\033[1;32m" <<"Begin of CollimateDecay..." << "\033[0m" << G4endl; 
#endif
...
void MyG4RadioactiveDecay::CollimateDecayProduct(G4DynamicParticle* daughter) {
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 1) {
    G4cout << "\033[1;32m" <<"CollimateDecayProduct for daughter "
	   << daughter->GetParticleDefinition()->GetParticleName() << "\033[0m" << G4endl; 
  }
#endif
...
G4ThreeVector MyG4RadioactiveDecay::ChooseCollimationDirection() const {
  // G4cout << "\033[1;32m" <<"MyG4RadioactiveDecay::ChooseCollimationDirection" << "\033[0m" << G4endl; 
...
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1)
    G4cout << "\033[1;32m" <<" ChooseCollimationDirection returns " << dir << "\033[0m" << G4endl; 
#endif

  return dir;
}

G4double MyG4RadioactiveDecay::ConvertingHalflifeToWidth(G4double halflife)
{
  return log(2) * CLHEP::hbar_Planck/(halflife*s);
}
